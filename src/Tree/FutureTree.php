...
abstract class FutureTree<+Tv, Tx as arraykey> {
	...
	public async function to_tree(): Awaitable<Tree<Tv, Tx>> {
		return new Tree(await async { return $this->forest->reduce(async (?Vector<this> $prev, Awaitable<this> $subtree) ==> {
			invariant(!is_null($prev), '');
			return $prev->add(await $subtree);
		}, new VectorIA(Vector{})) }, $this->v);
	}
